...
on tile[TILE_NUM] : buffered in port:1 p_rx =  XS1_PORT_1G;
on tile[TILE_NUM] : out port p_tx = XS1_PORT_1C;
on tile[TILE_NUM]: port p_led=XS1_PORT_4A;
on tile[TILE_NUM]: port p_button1=XS1_PORT_4C;
...
int main()
{
  ...
  par
  {
    on tile[TILE_NUM] : uart_rx(p_rx, rx_buffer, ARRAY_SIZE(rx_buffer), BAUD_RATE, 8, UART_TX_PARITY_EVEN, 1, c_chanRX);
    on tile[TILE_NUM] : uart_tx(p_tx, tx_buffer, ARRAY_SIZE(tx_buffer), BAUD_RATE, 8, UART_TX_PARITY_EVEN, 1, c_chanTX);
    on tile[TILE_NUM] : app_manager(c_chanTX,c_chanRX,c_process,c_end);
    on tile[TILE_NUM] : process_data(c_process, c_end);
  }
  return 0;
}
//::Main

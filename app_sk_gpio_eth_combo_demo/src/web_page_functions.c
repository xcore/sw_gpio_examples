...
int process_web_page_data(char buf[], int app_state, int connection_state)
{
  ...
  if (!user_choice || !(*user_choice)) {
	...
	gpio_data.led_0 = atoi(user_choice);
  ...
  if (!user_choice || !(*user_choice)) {
	...
	gpio_data.led_1 = atoi(user_choice);
  ...
  if (!user_choice || !(*user_choice)) {
	...
	gpio_data.led_2 = atoi(user_choice);
  ...
  if (!user_choice || !(*user_choice)) {
    ...
    gpio_data.led_3 = atoi(user_choice);
  ...
  if (!user_choice || !(*user_choice)) {
	...
	gpio_data.button_1 = atoi(user_choice);
  ...
  if (!user_choice || !(*user_choice)) {
	...
	gpio_data.button_2 = atoi(user_choice);

  if ((gpio_data.button_1) && (gpio_data.button_2))
	  button_id = 3;
  else if (gpio_data.button_1)
	  button_id = 1;
  else if (gpio_data.button_2)
	  button_id = 2;
  else
	  button_id = 0;

  gpio_data.temperature = 0;
  /* Send web page request to app_handler to apply LED and button status enquiry */
  set_gpio_state(c_gpio, &gpio_data);
  /* Fetch button press states and new temperature value */
  get_gpio_state(c_gpio, &gpio_data);
  return 0;
}
		...
		int ui_param)
{
	...
	{
	case 1:
	  if (gpio_data.led_0 == selected_value)
		select_flag = 1;
	  break;
	case 2:
	  if (gpio_data.led_1 == selected_value)
		select_flag = 1;
	  break;
	case 3:
	  if (gpio_data.led_2 == selected_value)
		select_flag = 1;
	  break;
	case 4:
	  if (gpio_data.led_3 == selected_value)
	    select_flag = 1;
	  break;
	case 5:
	case 6:
...
int read_temperature(char buf[], int app_state, int connection_state)
{
  if (!web_server_is_post(connection_state))
    ...
    selstr[42] = (gpio_data.temperature/10)+'0';
    selstr[43] = (gpio_data.temperature%10)+'0';
    strcpy(buf, selstr);
    return strlen(selstr);
  }
}
...
int get_button_state(char buf[], int app_state, int connection_state)
{
  ...
  if ((gpio_data.button_1) && (gpio_data.button_2) && (0x3 == button_id)) {
	char selstr[] = "<b>Both</b> Button 1 and Button 2 are pressed ";
	strcpy(buf, selstr);
	return strlen(selstr);
  }
  else if ((gpio_data.button_1) && (0x1 & button_id)) {
	char selstr[] = "<b>Button 1</b> is pressed";
	strcpy(buf, selstr);
	return strlen(selstr);
  }
  else if ((gpio_data.button_2) && (0x2 & button_id)) {
...
{
  chanend c_gpio = (chanend) ((app_state_t *) app_state)->c_gpio;
  reset_gpio_button_state(c_gpio, button_id);
  return 0;
}
